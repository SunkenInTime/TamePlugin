...
public class HytaleAssetStore<K, T extends JsonAssetWithMap<K, M>, M extends AssetMap<K, T>> extends AssetStore<K, T, M> {
   ...
   public void sendAssets(@Nonnull Consumer<Packet[]> packetConsumer) {
      if (this.packetGenerator != null) {
         Packet[] packets = this.cachedInitPackets == null ? null : (Packet[])this.cachedInitPackets.get();
         if (packets != null) {
   ...
   @Nonnull
   public static <T extends JsonAssetWithMap<String, M>, M extends AssetMap<String, T>> HytaleAssetStore.Builder<String, T, M> builder(Class<T> tClass, M assetMap) {
      return new HytaleAssetStore.Builder(String.class, tClass, assetMap);
   }

   @Nonnull
   public static <K, T extends JsonAssetWithMap<K, M>, M extends AssetMap<K, T>> HytaleAssetStore.Builder<K, T, M> builder(Class<K> kClass, Class<T> tClass, M assetMap) {
   ...
   static {
      ...
      SETUP_PACKET_CONSUMERS = new ObjectArrayList();
   }

   public static class Builder<K, T extends JsonAssetWithMap<K, M>, M extends AssetMap<K, T>> extends com.hypixel.hytale.assetstore.AssetStore.Builder<K, T, M, HytaleAssetStore.Builder<K, T, M>> {
   ...
   private class AssetStoreMonitorHandler implements AssetMonitorHandler {
      ...
      public AssetStoreMonitorHandler(String param2) {
      ...
      public void accept(Map<Path, EventKind> map) {
         ...
         while(var6.hasNext()) {
            ...
            case ENTRY_CREATE:
               if (Files.isDirectory(path, new LinkOption[0])) {
                  ...
                  try {
                     ...
                     ((Api)HytaleAssetStore.this.logger.at(Level.SEVERE).withCause(var14)).log("Failed to reload assets in directory: %s", path);
                  }
               } else {
            ...
            case ENTRY_DELETE:
               ...
               Iterator var9 = HytaleAssetStore.this.getAssetMap().getPathMap(this.packKey).values().iterator();
