   ...
   public void run() {
      if (!this.changed.getAndSet(false)) {
         ...
         try {
            ...
            Iterator var2 = this.paths.entrySet().iterator();

            while(var2.hasNext()) {
               Entry<Path, PathEvent> entry = (Entry)var2.next();
               entries.add(new SimpleEntry((Path)entry.getKey(), (PathEvent)entry.getValue()));
            }

            this.paths.clear();
            entries.sort(Comparator.comparingLong((value) -> {
               return ((PathEvent)value.getValue()).getTimestamp();
            }));
            ...
            ((Api)LOGGER.at(Level.SEVERE).withCause(var6)).log("Failed to run: %s", this);
         }
      }
