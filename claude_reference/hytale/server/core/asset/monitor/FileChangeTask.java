...
public class FileChangeTask implements Runnable {
   ...
   public void run() {
      try {
         ...
         ((Api)LOGGER.at(Level.SEVERE).withCause(new SkipSentryException(var4))).log("The asset file '%s' was deleted before we could load/update it!", this.path);
      } catch (Throwable var5) {
         ((Api)LOGGER.at(Level.SEVERE).withCause(var5)).log("Failed to handle file change %s", this.path);
      }
