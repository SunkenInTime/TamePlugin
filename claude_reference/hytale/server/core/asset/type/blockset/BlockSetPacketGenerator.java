...
public class BlockSetPacketGenerator extends AssetPacketGenerator<String, BlockSet, IndexedLookupTableAssetMap<String, BlockSet>> {
   ...
   public Packet generateInitPacket(@Nonnull IndexedLookupTableAssetMap<String, BlockSet> assetMap, Map<String, BlockSet> assets) {
      ...
      packet.blockSets = (Map)assetMap.getAssetMap().entrySet().stream().collect(Collectors.toMap(Entry::getKey, (entry) -> {
         return ((BlockSet)entry.getValue()).toPacket();
      }));
      return packet;
   }
   ...
   public Packet generateUpdatePacket(IndexedLookupTableAssetMap<String, BlockSet> assetMap, @Nonnull Map<String, BlockSet> loadedAssets, @Nonnull AssetUpdateQuery query) {
      ...
      packet.blockSets = (Map)loadedAssets.entrySet().stream().collect(Collectors.toMap(Entry::getKey, (entry) -> {
         return ((BlockSet)entry.getValue()).toPacket();
      }));
      return packet;
   }
