...
public class BlockSoundSet implements JsonAssetWithMap<String, IndexedLookupTableAssetMap<String, BlockSoundSet>>, NetworkSerializable<com.hypixel.hytale.protocol.BlockSoundSet> {
   ...
   protected void processConfig() {
      if (!this.soundEventIds.isEmpty()) {
         ...
         Iterator var1 = this.soundEventIds.entrySet().iterator();

         while(var1.hasNext()) {
            Entry<BlockSoundEvent, String> entry = (Entry)var1.next();
   ...
   static {
      CODEC = ((Builder)((Builder)((Builder)AssetBuilderCodec.builder(BlockSoundSet.class, BlockSoundSet::new, Codec.STRING, (blockSounds, s) -> {
         blockSounds.id = s;
      }, (blockSounds) -> {
         return blockSounds.id;
      }, (asset, data) -> {
         asset.data = data;
      }, (asset) -> {
         return asset.data;
      }).appendInherited(new KeyedCodec("SoundEvents", new EnumMapCodec(BlockSoundEvent.class, Codec.STRING)), (blockParticleSet, s) -> {
         blockParticleSet.soundEventIds = s;
      }, (blockParticleSet) -> {
         return blockParticleSet.soundEventIds;
      }, (blockParticleSet, parent) -> {
         blockParticleSet.soundEventIds = parent.soundEventIds;
      }).addValidator(Validators.nonNull()).addValidator(SoundEvent.VALIDATOR_CACHE.getMapValueValidator()).addValidator(SoundEventValidators.MONO_VALIDATOR_CACHE.getMapValueValidator()).addValidator(SoundEventValidators.ONESHOT_VALIDATOR_CACHE.getMapValueValidator()).metadata(UIDefaultCollapsedState.UNCOLLAPSED).add()).appendInherited(new KeyedCodec("MoveInRepeatRange", FloatRange.CODEC), (blockSounds, f) -> {
         blockSounds.moveInRepeatRange = f;
      }, (blockSounds) -> {
         return blockSounds.moveInRepeatRange;
      }, (blockSounds, parent) -> {
         blockSounds.moveInRepeatRange = parent.moveInRepeatRange;
      }).add()).afterDecode(BlockSoundSet::processConfig)).build();
      VALIDATOR_CACHE = new ValidatorCache(new AssetKeyValidator(BlockSoundSet::getAssetStore));
   }
}
