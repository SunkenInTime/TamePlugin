...
public class AmbienceFXMusic implements NetworkSerializable<com.hypixel.hytale.protocol.AmbienceFXMusic> {
   ...
   static {
      CODEC = ((Builder)((Builder)((Builder)BuilderCodec.builder(AmbienceFXMusic.class, AmbienceFXMusic::new).appendInherited(new KeyedCodec("Tracks", Codec.STRING_ARRAY), (ambienceFXMusic, strings) -> {
         ambienceFXMusic.tracks = strings;
      }, (ambienceFXMusic) -> {
         return ambienceFXMusic.tracks;
      }, (ambienceFXMusic, parent) -> {
         ambienceFXMusic.tracks = parent.tracks;
      }).addValidator(Validators.nonEmptyArray()).addValidator(new ArrayValidator(CommonAssetValidator.MUSIC)).add()).appendInherited(new KeyedCodec("Volume", Codec.FLOAT), (ambienceFXMusic, f) -> {
         ambienceFXMusic.decibels = f;
      }, (ambienceFXMusic) -> {
         return ambienceFXMusic.decibels;
      }, (ambienceFXMusic, parent) -> {
         ambienceFXMusic.decibels = parent.decibels;
      }).metadata(new UIEditor(new FormattedNumber((Double)null, " dB", (Integer)null))).addValidator(Validators.range(-100.0F, 10.0F)).add()).afterDecode(AmbienceFXMusic::processConfig)).build();
   }
}
