...
import com.hypixel.hytale.codec.schema.metadata.ui.UIEditor.FormattedNumber;
...
public class AmbienceFXAmbientBed implements NetworkSerializable<com.hypixel.hytale.protocol.AmbienceFXAmbientBed> {
   ...
   static {
      CODEC = ((Builder)((Builder)((Builder)((Builder)BuilderCodec.builder(AmbienceFXAmbientBed.class, AmbienceFXAmbientBed::new).appendInherited(new KeyedCodec("Track", Codec.STRING), (ambienceFXAmbientBed, s) -> {
         ambienceFXAmbientBed.track = s;
      }, (ambienceFXAmbientBed) -> {
         return ambienceFXAmbientBed.track;
      }, (ambienceFXAmbientBed, parent) -> {
         ambienceFXAmbientBed.track = parent.track;
      }).addValidator(Validators.nonNull()).addValidator(CommonAssetValidator.SOUNDS).addValidator(SoundFileValidators.STEREO).add()).appendInherited(new KeyedCodec("Volume", Codec.FLOAT), (ambienceFXAmbientBed, f) -> {
         ambienceFXAmbientBed.decibels = f;
      }, (ambienceFXAmbientBed) -> {
         return ambienceFXAmbientBed.decibels;
      }, (ambienceFXAmbientBed, parent) -> {
         ambienceFXAmbientBed.decibels = parent.decibels;
      }).metadata(new UIEditor(new FormattedNumber((Double)null, " dB", (Integer)null))).addValidator(Validators.range(-100.0F, 10.0F)).add()).appendInherited(new KeyedCodec("TransitionSpeed", new EnumCodec(AmbienceTransitionSpeed.class)), (ambienceFXAmbientBed, e) -> {
         ambienceFXAmbientBed.transitionSpeed = e;
      }, (ambienceFXAmbientBed) -> {
         return ambienceFXAmbientBed.transitionSpeed;
      }, (ambienceFXAmbientBed, parent) -> {
         ambienceFXAmbientBed.transitionSpeed = parent.transitionSpeed;
      }).documentation("How quickly to transition to this ambient bed and fade out any ambient beds that are stopping. For fading out stopping ambient beds, faster transitions take priority. Fade-ins are already fast by default.").add()).afterDecode(AmbienceFXAmbientBed::processConfig)).build();
   }
}
