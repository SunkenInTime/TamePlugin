...
public class AmbienceFXSoundEffect implements NetworkSerializable<com.hypixel.hytale.protocol.AmbienceFXSoundEffect> {
   ...
   static {
      CODEC = ((Builder)((Builder)((Builder)((Builder)BuilderCodec.builder(AmbienceFXSoundEffect.class, AmbienceFXSoundEffect::new).appendInherited(new KeyedCodec("ReverbEffectId", Codec.STRING), (ambienceFXSoundEffect, s) -> {
         ambienceFXSoundEffect.reverbEffectId = s;
      }, (ambienceFXSoundEffect) -> {
         return ambienceFXSoundEffect.reverbEffectId;
      }, (ambienceFXSoundEffect, parent) -> {
         ambienceFXSoundEffect.reverbEffectId = parent.reverbEffectId;
      }).addValidator(ReverbEffect.VALIDATOR_CACHE.getValidator()).add()).appendInherited(new KeyedCodec("EqualizerEffectId", Codec.STRING), (ambienceFXSoundEffect, s) -> {
         ambienceFXSoundEffect.equalizerEffectId = s;
      }, (ambienceFXSoundEffect) -> {
         return ambienceFXSoundEffect.equalizerEffectId;
      }, (ambienceFXSoundEffect, parent) -> {
         ambienceFXSoundEffect.equalizerEffectId = parent.equalizerEffectId;
      }).addValidator(EqualizerEffect.VALIDATOR_CACHE.getValidator()).add()).appendInherited(new KeyedCodec("IsInstant", Codec.BOOLEAN), (ambienceFXSoundEffect, b) -> {
         ambienceFXSoundEffect.isInstant = b;
      }, (ambienceFXSoundEffect) -> {
         return ambienceFXSoundEffect.isInstant;
      }, (ambienceFXSoundEffect, parent) -> {
         ambienceFXSoundEffect.isInstant = parent.isInstant;
      }).add()).afterDecode(AmbienceFXSoundEffect::processConfig)).build();
   }
}
