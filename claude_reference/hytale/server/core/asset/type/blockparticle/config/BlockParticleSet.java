...
public class BlockParticleSet implements JsonAssetWithMap<String, DefaultAssetMap<String, BlockParticleSet>>, NetworkSerializable<com.hypixel.hytale.protocol.BlockParticleSet> {
   ...
   static {
      CODEC = ((Builder)((Builder)((Builder)((Builder)((Builder)((Builder)AssetBuilderCodec.builder(BlockParticleSet.class, BlockParticleSet::new, Codec.STRING, (blockParticleSet, s) -> {
         ...
         return asset.data;
      }).documentation("Particle Systems that can be spawned in relation to block events.")).appendInherited(new KeyedCodec("Color", ProtocolCodecs.COLOR), (blockParticleSet, s) -> {
         blockParticleSet.color = s;
      }, (blockParticleSet) -> {
         return blockParticleSet.color;
      }, (blockParticleSet, parent) -> {
         blockParticleSet.color = parent.color;
      }).documentation("The colour used if none was specified in the particle settings.").add()).appendInherited(new KeyedCodec("Scale", Codec.FLOAT), (blockParticleSet, f) -> {
         blockParticleSet.scale = f;
      }, (blockParticleSet) -> {
         return blockParticleSet.scale;
      }, (blockParticleSet, parent) -> {
         blockParticleSet.scale = parent.scale;
      }).documentation("The scale of the particle system.").add()).appendInherited(new KeyedCodec("PositionOffset", ProtocolCodecs.VECTOR3F), (blockParticleSet, s) -> {
         blockParticleSet.positionOffset = s;
      }, (blockParticleSet) -> {
         return blockParticleSet.positionOffset;
      }, (blockParticleSet, parent) -> {
         blockParticleSet.positionOffset = parent.positionOffset;
      }).documentation("The position offset from the spawn position.").add()).appendInherited(new KeyedCodec("RotationOffset", ProtocolCodecs.DIRECTION), (blockParticleSet, s) -> {
         blockParticleSet.rotationOffset = s;
      }, (blockParticleSet) -> {
         return blockParticleSet.rotationOffset;
      }, (blockParticleSet, parent) -> {
         blockParticleSet.rotationOffset = parent.rotationOffset;
      }).documentation("The rotation offset from the spawn rotation.").add()).appendInherited(new KeyedCodec("Particles", new EnumMapCodec(BlockParticleEvent.class, Codec.STRING)), (blockParticleSet, s) -> {
         blockParticleSet.particleSystemIds = s;
      }, (blockParticleSet) -> {
         return blockParticleSet.particleSystemIds;
      }, (blockParticleSet, parent) -> {
