...
public final class TransformingClassLoader extends URLClassLoader {
   ...
   private Class<?> transformAndDefine(String name, String internalName, byte[] classBytes, URL resource) {
      if (!isSecureClass(name)) {
         Iterator var5 = this.transformers.iterator();

         while(var5.hasNext()) {
            ClassTransformer transformer = (ClassTransformer)var5.next();
            ...
            try {
               ...
               var10000.println("[EarlyPlugin] Transformer " + var10001 + " failed on " + name + ": " + var8.getMessage());
   ...
   private static boolean isSecureClass(@Nonnull String name) {
      Iterator var1 = SECURE_PACKAGE_PREFIXES.iterator();
