...
public final class EarlyPluginLoader {
   ...
   public static void loadEarlyPlugins(@Nonnull String[] args) {
      ...
      if (!urls.isEmpty()) {
         pluginClassLoader = new URLClassLoader((URL[])urls.toArray((x$0) -> {
            return new URL[x$0];
         }), EarlyPluginLoader.class.getClassLoader());
         var2 = ServiceLoader.load(ClassTransformer.class, pluginClassLoader).iterator();

         while(var2.hasNext()) {
            ClassTransformer transformer = (ClassTransformer)var2.next();
   ...
   private static List<Path> parseEarlyPluginPaths(@Nonnull String[] args) {
      ...
      for(int i = 0; i < args.length; ++i) {
         ...
         if (args[i].equals("--early-plugins") && i + 1 < args.length) {
            String[] var8 = args[i + 1].split(",");
            int var9 = var8.length;

            for(var5 = 0; var5 < var9; ++var5) {
               String pathStr = var8[var5];
   ...
   private static void collectPluginJars(Path path, List<URL> urls) {
      if (Files.isDirectory(path, new LinkOption[0])) {
         try {
            DirectoryStream stream = Files.newDirectoryStream(path, "*.jar");
            ...
            var10000.println("[EarlyPlugin] Failed to scan directory " + var10001 + ": " + var7.getMessage());
         }
